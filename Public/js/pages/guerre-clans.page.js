...
function renderActiveWar(activeWar) {
  ...
  // --- PATCH: Préserver la valeur de l'input de mise ---
  let betInputValue = "";
  let hadFocus = false;
  const oldInput = document.getElementById("betAmountInput");
  if (oldInput && typeof oldInput.value === "string") {
    betInputValue = oldInput.value;
  ...
  if (canBet) {
    bindBetForm(activeWar);
    // Réinjecter la valeur précédente de l'input de mise si existante
    const betInput = document.getElementById("betAmountInput");
    if (betInput && betInputValue) {
      betInput.value = betInputValue;
      if (hadFocus) {
        betInput.focus();
        // Place le curseur à la fin
        betInput.setSelectionRange(
          betInput.value.length,
          betInput.value.length,
        );
      }
    }
...
async function init() {
  ...
  socket.on("clanwar:update", (active) => {
    ...
    // Toujours rafraîchir l'historique à chaque tick
    loadHistoryOnly();
  });
  ...
  // Ajout d'un refresh périodique même sans tick serveur
  setInterval(() => {
    loadHistoryOnly();
  }, 15000);
  ...
  // Fonction utilitaire pour rafraîchir uniquement l'historique
